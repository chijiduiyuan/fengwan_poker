...
module.exports = function () {
    ...
    this.baseBet_SG     = 0; //三公基础底注    
    this.baseBet_COW    = 0; //牛牛基础底注
    

    this.bankerUid_SG = 0; //三公庄家 uid
    this.bankerUid_COW = 0; //牛牛 庄家 uid

   
    this.maxCallBankerUids_SG  = []; //三公最大叫庄倍率 玩家数组
    ...
    //callBankerStep_SG 三公叫庄
    //showCardStep_SG 三公搓牌
    //搓牌结束 结算三公 暂停5秒
    ...
    this.init = function(roomData){      
        ...
        G.trace('[init] game_cowWater roomId：'+this.roomId+' seqNum='+this.seqNum);
        G.trace(roomData);
        ...
        this.baseBet_SG = this.baseBet_COW = this.roomInfo.baseBet*1; //底注

        this.roomInfo.maxCallBankerOdds_SG  = 0; //三公最大叫庄倍率
        ...
        for(var i in playerInfo){
            this.players[i] = {
                ...
                "callBankerOdds_SG" : -1, //三公叫庄倍率 -1表示没有叫过 0 表示没有资格 >0表示叫庄倍率
                "callBankerOdds_COW" : -1,//牛牛叫庄倍率

                "isShowCard_SG" : false,//三公是否搓牌过  
    ...
    this.doUpdate = function(){
        ...
        while(opArr.length>0){
            ...
            case 'showCard_SG'://三公搓第三张牌               
                this.showCard_SG(opInfo.uid);
                break;
            case 'callBanker_SG': //三公叫庄             
                this.callBankerOdds_SG(opInfo.uid,opInfo.odds);
                break;     
    ...
    this.nextRound = function(){
        ...
        case 'step_ready' ://游戏开始 
            G.trace('进入 三公叫庄');
            this.roomInfo.curRound = 'step_callBanker_SG';
            //设置超时进入下一轮
            this.roomInfo.ts_time = G.time()+CFG.game_cowWater.callBankerTimeout_SG;
            this.setTimer( this.nextRound,CFG.game_cowWater.callBankerTimeout_SG );
            break;
        case 'step_callBanker_SG':
            //处理三公叫庄结果
            G.trace('处理三公叫庄结果');
                        
            if(this.roomInfo.maxCallBankerOdds_SG == 0 ){//没人叫庄则 在允许叫庄的玩家列表中随机            
                this.roomInfo.maxCallBankerOdds_SG = 1;         
                for(var i in this.players){
                    this.maxCallBankerUids_SG.push(i);
                }
            }


            //最大叫庄玩家列表里面随机取一个
            this.bankerUid_SG = G.arrayRand(this.maxCallBankerUids_SG);    
            //庄家位置更新 
            this.roomInfo.bankerSite = this.players[this.bankerUid_SG].site;

            //发送三公叫庄结果给客户端
            this.toClient({
                "op"    :"sg_callBanker",
                "uid"   :this.bankerUid_SG, //三公庄家
                "maxCallBankerUids_SG" : this.maxCallBankerUids_SG,//最大叫庄玩家列表
                "odds"  :this.roomInfo.maxCallBankerOdds_SG //叫庄倍率
            });
            ...
            //进入三公搓牌
            G.trace('进入 三公搓牌');
            this.roomInfo.curRound = 'step_showCard_SG';   
            //设置超时进入下一轮
            this.roomInfo.ts_time = G.time()+CFG.game_cowWater.showCardTimeout_SG;
            this.setTimer( this.nextRound,CFG.game_cowWater.showCardTimeout_SG+2 ); //后台增加2秒用做网络延迟      
            break;
        case 'step_showCard_SG':
            ...
            //进入三公checkout
            G.trace('进入 三公checkout');
            this.roomInfo.curRound = 'step_checkout_SG';   
            this.checkout_SG();
            this.nextRound();
            return;
            //this.roomInfo.curRound = 'step_checkout_SG';   
            //this.roomInfo.ts_time = G.time()+CFG.game_cowWater.checkoutTimeout_SG;
            //this.setTimer( this.nextRound,CFG.game_cowWater.checkoutTimeout_SG );  
           // return;       
            break;
        case 'step_checkout_SG':
            //进入牛牛叫庄
            G.trace('进入 牛牛叫庄');
            this.roomInfo.curRound = 'step_callBanker_COW';       
            this.roomInfo.ts_time = G.time()+CFG.game_cowWater.callBankerTimeout_COW;    
            this.setTimer( this.nextRound,CFG.game_cowWater.callBankerTimeout_COW );   
            break;
        case 'step_callBanker_COW':  
            ...
            //设置超时进入下一轮
            this.roomInfo.ts_time = G.time()+CFG.game_cowWater.putCardTimeout_COW;    
            this.setTimer( this.nextRound,CFG.game_cowWater.putCardTimeout_COW+2  );  //后台增加2秒用做网络延迟
            break;
     ...
     * @access public
     */    
    this.sendCard_SG = function(){
        var tmpCard,cardKey;
        //每个人发2张
        var preNum = 2;
        var showCards = {};//每人前2张显示给所有人看
        G.trace('三公发牌 每个人发2张显牌,1张底牌'); 
        ...
        for(var p in this.players){       
            ...
            for(var i=0;i<preNum;i++){
                ...
                showCards[p].push( G.cardToNum(tmpCard,'cowWater') );  

                this.players[p]['cards'].push(tmpCard);                
            }
        }         
        G.trace('每人前2张牌显示给所有人看');
        G.trace(showCards);
        this.toClient({
            "op"        :'sg_showCards',                            
            "showCards" : showCards
        ...
        for(var p in this.players){                               
            ...
            this.players[p]['cards'].push(tmpCard);                           
            this.toClient({
                "op"        :'sg_self_showCards', //第一轮底牌                           
                "cards"     :G.cardToNum(this.players[p]['cards'],'cowWater')
            } ,this.players[p]['uid'] );
        }
   ...
   /**
     ...
     * @access public
     */ 
    this.showCard_SG = function(uid){

        this.players[uid].isShowCard_SG = true;
        
         //如果所有人都已经搓牌过了则进入第2阶段
        var allDone = true;
        for(var i in this.players){
            if(!this.players[i].isShowCard_SG){
                allDone = false;
                break;
            } 
        }
     ...
     * @access public
     */ 
    this.callBankerOdds_SG = function(uid,odds){

        G.trace('玩家三公叫庄 uid='+uid+' odds='+odds);

        this.players[uid].callBankerOdds_SG = odds; 

        //保存最大叫庄倍率和玩家
        if(odds > this.roomInfo.maxCallBankerOdds_SG){
            //更新最大叫庄
            this.roomInfo.maxCallBankerOdds_SG = odds;
            this.maxCallBankerUids_SG = [uid];
        }else if(odds == this.roomInfo.maxCallBankerOdds_SG){
            //同样是最大叫庄 增加玩家
            this.maxCallBankerUids_SG.push(uid);
        }
        ...
        var allDone = true;
        for(var i in this.players){
            if(this.players[i].callBankerOdds_SG===-1){
                allDone = false;
                break;
            } 
        }
     ...
     * @access public
     */ 
    // this.callBankerOddsTimeout_SG = function(uid,odds){

    //     //发送给客户端 叫庄结果
        
    //     //进入下一阶段
    //     this.nextRound();
    // }//callBankerOddsTimeout_SG
     ...
     * @access public
     */ 
    // this.callBankerOddsTimeout_COW = function(uid,odds){
    //     //发送给客户端 叫庄结果
    //     this.nextRound();
    //     //进入下一阶段
    // }//callBankerOddsTimeout_SG
    ...
    //牛牛摆牌超时
    // this.putCardTimeout = function(uid,cards){        
     ...
     * 
     */
    this.getCardType_SG = function(uid){
        //5+2 = 7张牌 评出最大的牌型        
        G.trace('[getCardType_SG] 取牌型 大小 uid='+uid);
        var cards = [];
        for(var ci=0;ci<3;ci++){//三公比牌只要前3张
            cards.push(this.players[uid]['cards'][ci]);            
        }
        G.trace('[getCardType_SG] 手牌 uid='+uid);
        G.trace(cards);        

        //3条
        if( cards[0].n==cards[1].n && cards[0].n==cards[2].n ){
            G.trace('三条 '+cards[0].n);
            //主牌型评分
            var tmpNum = this.cType_SG.THREE_CARD *1000000
                        + cards[0].n * 100;            
            return {
                "type"      : "THREE_CARD",                
                "point"     : tmpNum,
        ...
        if( this.isJQK(cards) ){
            G.trace('三公');
            //主牌型评分 三公没有在比大小
            var tmpNum = this.cType_SG.JQK_CARD *1000000;            
            return {
                "type"      : "JQK_CARD",                
                "point"     : tmpNum,
                "cards"     : cards
        ...
        var cardPoint = 0;//牌型点数
        var pointArr = [];//计算副牌型积分用
        for(var i in cards){
            if(cards[i].n>10){//10 J Q K 都是10
                //JQK 相等
                pointArr.push(11);
                continue;
            }
            pointArr.push(cards[i].n);            
            cardPoint += cards[i].n;
        }
        cardPoint = cardPoint % 10; 
        var secondPoint = pointArr[2]*100
                        + pointArr[1]*10
                        + pointArr[0]*1;
        //十点
        if(cardPoint == 0){
            G.trace('十点');
            //主牌型评分 没有在比大小
            var tmpNum = this.cType_SG.TEN_CARD *1000000
                        + secondPoint;         
            return {
                "type"      : "TEN_CARD",                
                "point"     : tmpNum,
        ...
        if(cardPoint == 9){
            G.trace('九点');
            //主牌型评分 没有在比大小
            var tmpNum = this.cType_SG.NINE_CARD *1000000
                        + secondPoint;            
            return {
                "type"      : "NINE_CARD",                
                "point"     : tmpNum,
        ...
        //其他点数
        G.trace('其他点数');
        var tmpNum = this.cType_SG.OTHER_CARD *1000000
                        + cardPoint*10000
                        + secondPoint;      
        return {
            "type"      : "OTHER_CARD",            
    ...
    this.getCardType_COW = function(uid){
        ...
        var maxC = -1;
        var tempC = -1
        for(var ci=0;ci<5;ci++){//三公比牌要全部5张
    ...
    this.checkout_SG = function(){
        ...
        var baseBet = this.baseBet_SG*this.roomInfo.maxCallBankerOdds_SG;
       
        G.trace('基础赔率('+baseBet
            +')=三公基础底注('+this.baseBet_SG
            +')*三公庄家倍数('+this.roomInfo.maxCallBankerOdds_SG
            +') ');
        for(var ui in this.players){
            rs[ui] = {};
            rs[ui]['thirdCard'] = G.cardToNum(this.players[ui].cards[2],'cowWater');//第三张底牌列表
         
            cardType = this.getCardType_SG(ui);            
             // "type"      : this.cType.THREE_CARD,                
             //    "point"     : tmpNum,
             //    "cards"     : cards
            this.players[ui]['point_SG'] = cardType.point;
            this.players[ui]['cardType_SG'] = cardType.type;
            this.players[ui]['cards_SG'] = G.clone(cardType.cards);
       
            if(  Number.isInteger(cardType.cardPoint) ){
                this.players[ui]['cardPoint_SG'] = cardType.cardPoint;    
            }

            //手牌倍率
            this.players[ui]['cardOdds_SG'] = CFG.game_cowWater.odds_SG[cardType.type];
       
            if(this.bankerUid_SG==ui){
        ...
        for(var j in this.players){
            if(this.bankerUid_SG==j){
                continue;
            }
            if(cardPointForBanker == this.players[j]['point_SG']){//打和
                //三公和牛牛要是打和，是庄家赢底注（标庄乘倍后的底注）。而不是打和
                
                G.trace('打和 庄家赢底注 cardPointForBanker='+cardPointForBanker+' player:'+this.players[j]['point_SG']);
                ...
                this.players[j].loseBet_SG = costBet;

                G.trace('玩家打和输了 uid='+j+' costBet='+costBet);

                bankerWinBet += costBet;
                G.trace('庄家赢了 bankerWinBet='+bankerWinBet);

                continue;
            }     

            if(cardPointForBanker>this.players[j]['point_SG']){
                //庄家赢钱   按庄家的手牌倍率算             
                G.trace('庄家赢钱   按庄家的手牌倍率算='+this.players[this.bankerUid_SG]['cardOdds_SG']);
                var costBet = baseBet* this.players[this.bankerUid_SG]['cardOdds_SG'];
                ...
                G.trace('玩家输了 uid='+j+' costBet='+costBet);

                bankerWinBet += costBet;
                G.trace('庄家赢了 bankerWinBet='+bankerWinBet);
            }else{
                //普通玩家赢钱  按普通玩家的手牌倍率算     
                G.trace('普通玩家赢钱 按普通玩家的手牌倍率算 ='+this.players[j]['cardOdds_SG']);                          
                var costBet = baseBet* this.players[j]['cardOdds_SG'];  
                playerWinBet += costBet;        
                playerWinList[j] = costBet;
                G.trace('玩家赢了 uid='+this.players[j].bet+' costBet='+costBet);
                G.trace('所有普通玩家赢了 playerWinBet='+playerWinBet);
            }
        }

        //庄家先赢钱 在分钱给输给玩家的
        this.players[this.bankerUid_SG].bet += bankerWinBet;
        this.players[this.bankerUid_SG].winBet_SG = bankerWinBet;

        if(playerWinBet>this.players[this.bankerUid_SG].bet){
            //庄家爆厂
            playerWinBet = this.players[this.bankerUid_SG].bet;

            G.trace('庄家爆厂 playerWinBet='+playerWinBet);
            ...
            if(lostBet>0){
                ...
                var nextUid = this.nextBankerPlayer(this.bankerUid_SG,playerWinList);
                tmp[nextUid] += lostBet;
        ...
        this.players[this.bankerUid_SG].bet -= playerWinBet;
        this.players[this.bankerUid_SG].loseBet_SG = playerWinBet;

        //庄家把钱分给赢钱玩家
        for(var v in playerWinList){            
            this.players[v].bet += playerWinList[v]; 
            this.players[v].winBet_SG = playerWinList[v]; 
        }
        
        var winBet_SG = 0;
        for(var pi in this.players){ 
            //发送三公结算结果给客户端 赢得筹码必须是抽水过的
            winBet_SG = this.players[pi].winBet_SG ;
            
            //牛家水这边，只有庄家赢了才抽水，返回给客户端的显示
            if(this.bankerUid_SG == pi){
                console.log('牛家水这边，只有庄家赢了才抽水， this.roomInfo.costScale='+this.roomInfo.costScale);
                console.log('self.bankerUid_SG='+self.bankerUid_SG+' this.bankerUid_SG:'+this.bankerUid_SG);
                console.log('庄家 winBet_SG='+this.players[pi].winBet_SG);
                console.log('庄家 lostBet_SG='+this.players[pi].loseBet_SG);
                if(winBet_SG>this.players[pi].loseBet_SG){
                    console.log('三公 庄家赢钱了 开始抽水后给客户端');
                    winBet_SG -= Math.floor(winBet_SG*this.roomInfo.costScale);
                }else{
                    console.log('三公 庄家输钱了 不抽水');
                }
                
            }
            rs[pi]['cardType_SG']    = this.players[pi].cardType_SG;
            rs[pi]['cardPoint_SG']   = this.players[pi].cardPoint_SG;
            rs[pi]['restBet']    = this.players[pi].bet;
            rs[pi]['winBet_SG']  = winBet_SG;
            rs[pi]['loseBet_SG'] = this.players[pi].loseBet_SG;   
            //第三张手牌         
        }
        rs[this.bankerUid_SG]['isBanker'] = 1;
    ...
    this.checkout_COW = function(){
        ...
        var checkoutRS = {
            ...
            "op"       :"cow_checkout" ,
            "roomId"   :this.roomId,
            "bankerUid_SG" : this.bankerUid_SG,
            "bankerUid_COW" : this.bankerUid_COW,
            "maxCallBankerOdds_SG" : this.roomInfo.maxCallBankerOdds_SG,
        ...
        for(var pi in this.players){ 
            ...
            //PHP请求参数组织
            balance = curPlayer.winBet_SG+curPlayer.winBet_COW
                        - curPlayer.loseBet_SG-curPlayer.loseBet_COW;
            balance_SG  = curPlayer.winBet_SG- curPlayer.loseBet_SG;
            balance_COW = curPlayer.winBet_COW- curPlayer.loseBet_COW;
            checkoutRS.result[pi] = {                     
                "cardType_SG"   : curPlayer.cardType_SG,//三公幸运牌型                
                "cards_SG"      : G.cardToNum(curPlayer.cards_SG,'cowWater'),//三公幸运牌型 牌列表                
            ...
            //delete(C['room_cowWater_users'][pi]);//清除玩家缓存
            G.roomObj.delRoomUser(pi);
        }
        //delete(C['room_cowWater'][this.roomId]);
        ...
        G.phpPost(checkoutRS,function(postRs){
            ...
            //自动开始下一局
            G.trace('自动开始下一局');
            G.trace(CFG.game_cowWater.nextStartTimeout);
            G.roomObj.nextStartByRoomId(self.roomId,CFG.game_cowWater.nextStartTimeout);
